document.addEventListener('DOMContentLoaded', function() {
  ...
  // This variable should be set from the PHP side
  const userHasLocation = typeof hasLocation !== 'undefined' && hasLocation === true;
  
  console.log("Location initialization - User has location:", userHasLocation);
  
  // Only proceed with map initialization if location is not set
  if (!userHasLocation) {
    ...
    if (submitManualLocation) {
      submitManualLocation.addEventListener('click', function () {
        const btn = this;
    ...
    console.log("User already has a location set. Skipping location initialization.");
    
    // Optional: If you want to display the user's current location without 
    // initializing the full location selection functionality
    if (typeof userLocation !== 'undefined') {
      console.log("Displaying user's saved location:", userLocation);
      // Here you could display the user's current location somewhere on the page
      // For example, update a location display element
      const userLocationDisplay = document.getElementById('userLocationDisplay');
      if (userLocationDisplay) {
        userLocationDisplay.textContent = userLocation.address;
      }
    }
  }
});